...
export class CricketService {
  ...
  private async saveMatchToDatabase(matchData: any): Promise<void> {
    try {
      ...
      // IMPORTANT: Matches can be completed early (not all overs played) due to:
      // - Rain/DLS (Duckworth-Lewis-Stern)
      // - Target reached (team 2 reaches target before all overs)
      // - All out (team all out before max overs)
      // - Other reasons (abandoned, etc.)
      ...
      // If API/transformer says completed, trust it - don't override based on scorecard
      // The transformer already checks status field, state_id, and note field
      // If it marked as completed, the match is completed (even if not all overs played)
      if (finalStatus === 'completed') {
        // Match is completed - ensure matchEnded is true
        matchEnded = true;
        this.logger.log(`Match ${matchData.matchId}: Marked as completed by API/transformer`, 'CricketService');
      }
    ...
    try {
      ...
      if (sportMonksToken) {
        try {
          ...
          // IMPORTANT: Even if a match comes from livescores endpoint, if it's marked as completed
          // by the transformer (based on status field, note field, etc.), exclude it
          const liveMatches = transformedMatches.filter((match) => {
            // First check: if status is explicitly completed, ALWAYS exclude (even from livescores)
            // The transformer has already checked status field, state_id, and note field
            // If it marked as completed, the match is completed regardless of which endpoint it came from
            if (match.status === 'completed') {
              this.logger.log(`Excluding match ${match.matchId}: transformer marked as completed (status field, note, or state_id indicates completion)`, 'CricketService');
              return false;
            }
            
            // Second check: if match has ended, exclude it
            if (match.matchEnded) {
              this.logger.log(`Excluding match ${match.matchId}: match has ended`, 'CricketService');
              return false;
            }
            
            // CRITICAL: If status is 'live', include it (matches from livescores endpoint)
  ...
  /**
   * Get completed matches - uses new CompletedMatchService
   */
  ...
  /**
   * OLD METHOD - DEPRECATED - Use getResults() instead
   */
  async getResults_OLD(filters: GetMatchesDto) {
    const { page = 1, limit = 20, format, series, startDate, endDate } = filters;
